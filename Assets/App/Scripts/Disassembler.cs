...
namespace StudioKurage.Emulator.Gameboy
{
    public class Disassembler
    {
        ...
        public string Disassemble ()
        {
            ...
            if (assembly.Contains ("0x{0:X4}")) {
                assembly = string.Format (assembly, cpu.mmu.rw(nextPc));
            } else if (assembly.Contains ("0x{0:X2}")) {
                assembly = string.Format (assembly, cpu.mmu.rb(nextPc));
            }

            return string.Format("0x{0:X4} 0x{1:X4} {2}", cpu.pc, opcode, assembly);
        }

        void Disassemble (ushort opcode, out ushort printOpcode, out string assembly)
        {
            if ((opcode & 0x00ff) == 0xcb) {
                opcode >>= 8;
                printOpcode = (ushort)(0xcb00 | opcode);
                assembly = prefixedInstructions [opcode];
            } else {
                opcode &= 0x0ff;
                printOpcode = opcode;
        ...
        public static string[] prefixedInstructions = new string[] {
            // 0x00 .. 0x0F
            "RLC B",
            "RLC C",
            "RLC D",
